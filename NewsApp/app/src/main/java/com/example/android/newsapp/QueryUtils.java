    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                JSONArray contributorsArray = currentNews.getJSONArray("tags");
                JSONObject currentContributor;
                String contributor = new String() ;


                if(contributorsArray.length() != 0) {
                    currentContributor = contributorsArray.getJSONObject(0);
                ...
                // Create a new {@link Earthquake} object with the url, section, title,
                // and date from the JSON response.
                News newsObject = new News(url, section, title, date, contributor);
